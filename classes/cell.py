...
class Cell:
    ...
    def __init__(self, 
        ...
        self._adjacent_cells = self.get_adjacent_cells(1)  
        self._bordering_cells = {**self.adjacent_cells, **self.get_adjacent_cells(2,start_index=7) }
        self._boundaries_type = boundaries_type  
    ...
    @property
    def bordering_cells(self):
        """返回边界单元格列表"""
        return self._bordering_cells
    
    @property
    def boundaries_type(self):
        """获取边界类型。该函数返回一个表示边界类型的值。"""
        return self._boundaries_type
    ...
    def get_coordinate(self,return_typle:Union[str,tuple] = str):
        """返回单元格的坐标，默认返回字符串格式"""
        coordinate_tuple = (Constants.get_column_letter(self.coordinate[0]),self.coordinate[1])
        return f"{coordinate_tuple[0]}{coordinate_tuple[1]}" if return_typle == str else coordinate_tuple

    def get_adjacent_cells(self, circle_number,start_index = 1):
        """根据圈数返回相邻单元格列表
        ...
        if circle_number == 1:
            if c_x%2 == 1:
                adjacent_cell_dict = {adj_index:(ajd_c if check_adj_by_border(ajd_c) else None) for adj_index,ajd_c in enumerate(get_adj_by_change(self.coordinate,Constants.BoardCst.ODD_δx6),start_index)}
            else:
                    adjacent_cell_dict = {adj_index:(ajd_c if check_adj_by_border(ajd_c) else None) for adj_index,ajd_c in enumerate(get_adj_by_change(self.coordinate,Constants.BoardCst.EVEN_δx6),start_index)}
                    
            return adjacent_cell_dict
        
        elif circle_number == 2:
            adjacent2_cell_dict = {adj_index:(ajd_c if check_adj_by_border(ajd_c) else None) for adj_index,ajd_c in enumerate(get_adj_by_change(self.coordinate,(Constants.BoardCst.ODD_δx12 if c_x%2 ==1 else Constants.BoardCst.EVEN_δx12)),start_index)}
    ...
    def get_current_army(self):
        # 返回单元格的驻扎军队
        return self.occupier

    def change_current_army(self, new_army):
        # 更新单元格的驻扎军队
...
if __name__ == "__main__":
    ...
    # print(test_cell.adjacent_cells)
    # print(test_cell.bordering_cells)
    print(test_cell.boundaries_type)
    print(test_cell.get_coordinate())